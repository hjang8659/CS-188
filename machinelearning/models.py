...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        self.lst = [self.w1, self.w2, self.b1, self.b2]
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(batch_size):
            temp = list(nn.gradients(self.get_loss(x, y), self.lst))
            for i in range(len(self.lst)):
                self.lst[i].update(temp[i], learning_rate)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                gw1, gb1, gw2, gb2, gw3, gb3 = nn.gradients(self.get_loss(
                    x, y), [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
                ...
                self.w3.update(gw3, learning_rate)
                self.b3.update(gb3, learning_rate)
                if dataset.get_validation_accuracy() >= 0.975:
                    return
                if dataset.get_validation_accuracy() >= 96:
