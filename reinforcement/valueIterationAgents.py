...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            for j in self.mdp.getStates():
                Q_val = [n_inf]
                if self.mdp.isTerminal(j) == 0:
                    for k in self.mdp.getPossibleActions(j):
                        Q_val.append(self.getQValue(j, k))
                    val_dict[j] = max(Q_val)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for i in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            result += reward + (self.discount * (self.values[i[0]] * i[1]))
        return result
    ...
    def computeActionFromValues(self, state):
        ...
        for i in self.mdp.getPossibleActions(state):
            result[i] = self.computeQValueFromValues(state, i)
