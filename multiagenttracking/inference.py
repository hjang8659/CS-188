...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    lst = []
    obs_dis = range(X_RANGE + Y_RANGE + (MAX_NOISE - 1))
    ...
    variableDomainsDict[OBS0] = [i for i in obs_dis]
    variableDomainsDict[OBS1] = [i for i in obs_dis]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for i in eliminationOrder:
            in_factor, j_factor = joinFactorsByVariable(factor, i)
            factor = in_factor

            if len(j_factor.unconditionedVariables()) > 1:
                factor.append(eliminate(j_factor, i))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        dist = manhattanDistance(ghostPosition, jailPosition)
        if noisyDistance == None and dist == 0:
            return 1
        elif (noisyDistance != None and dist == 0) or (noisyDistance == None and dist != 0):
            return 0
        else:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            self.beliefs[i] *= self.getObservationProb(
                observation, gameState.getPacmanPosition(), i, self.getJailPosition())
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            prev = self.beliefs[i]
            new_pos_dist = self.getPositionDistribution(gameState, i)
            for j in self.allPositions:
                updated_belief[j] += (new_pos_dist[j] * prev)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            lst = i % len(self.legalPositions)
            self.particles.append(self.legalPositions[lst])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.particles:
            P = self.getObservationProb(
                observation, gameState.getPacmanPosition(), i, self.getJailPosition())
            if i not in update.keys():
                update[i] = P
            update[i] += P
        ...
        if not update.total():
            self.initializeUniformly(gameState)
            update = self.getBeliefDistribution()

        self.particles = [update.sample() for i in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [self.getPositionDistribution(gameState, i).sample()
                          for i in self.particles]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in self.particles:
            ...
            for j in range(self.numGhosts):
                P *= self.getObservationProb(
                    observation[j], gameState.getPacmanPosition(), i[j], self.getJailPosition(j))
            belief[i] += P
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            newParticle = [self.getPositionDistribution(
                gameState, oldParticle, i, self.ghostAgents[i]).sample() for i in range(self.numGhosts)]
            """*** END YOUR CODE HERE ***"""
