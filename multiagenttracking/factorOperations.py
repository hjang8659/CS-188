...
def joinFactors(factors: List[Factor]):
    ...
    "*** YOUR CODE HERE ***"
    # CV = conditional variable, UV = unconditional variable
    CV = []
    UV = []
    factor_lst = []
    ...
    for i in factor_lst:
        for j in i.conditionedVariables():
            if j not in CV:
                CV.append(j)
        for k in i.unconditionedVariables():
            if k not in UV:
                UV.append(k)
    ...
    VDD = factor_lst[0].variableDomainsDict()
    new_factor = Factor(UV, CV, VDD)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        "*** YOUR CODE HERE ***"
        # CV = conditional variable, UV = unconditional variable, VDD = variable domain dictionary
        CV = factor.conditionedVariables()
        UV = list(factor.unconditionedVariables())
        VDD = factor.variableDomainsDict()
        UV = [i for i in UV if i != eliminationVariable]
        new_factor = Factor(UV, CV, VDD)

        for i in new_factor.getAllPossibleAssignmentDicts():
            new_factor.setProbability(i, 0)

        for i in factor.getAllPossibleAssignmentDicts():
            P = new_factor.getProbability(i)
