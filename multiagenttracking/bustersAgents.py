    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        succ_pos, pred_ghost_pos, min_ghost_dist, pred_ghost_dist = [], [], [], []

        for i in legal:
            succ_pos.append(Actions.getSuccessor(pacmanPosition, i))

        for i in livingGhostPositionDistributions:
            pred_ghost_pos.append(i.argMax())

        for i in succ_pos:
            for j in pred_ghost_pos:
                pred_ghost_dist.append(self.distancer.getDistance(i, j))
            min_ghost_dist.append(min(pred_ghost_dist))

        return legal[min_ghost_dist.index(min(min_ghost_dist))]
        "*** END YOUR CODE HERE ***"
